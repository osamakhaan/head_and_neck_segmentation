    ...
    def __getitem__(self, index):
        ...
        if not self.ist: #[::2, ::2, ::2]
            ...
            if self.monai_transform=="crop1":
                ...
                f = 1.4
                size = [int(f*78), int(f*206), int(f*164)] # average along all 3 dims in traindataset
                transform = Compose([
                ...
                data_dict = {"image":img, "label":label}
                f = 1.2
                size = [int(f*78), int(f*206), int(f*164)]
                transform = Compose([
                    Resized(keys=["image", "label"], spatial_size=size),
                ]
                )
                transformed_data_dict = transform(data_dict)
                return transformed_data_dict['image'], transformed_data_dict['label'], flag 
            elif self.monai_transform=="crop3":
                data_dict = {"image":img, "label":label}
                f = 1.0
                size = [int(f*78), int(f*206), int(f*164)]
                transform = Compose([
                ...
                transformed_data_dict = transform(data_dict)
                return transformed_data_dict['image'], transformed_data_dict['label'], flag   
            elif self.monai_transform=="crop4":
                data_dict = {"image":img, "label":label}
                f = 0.8
                size = [int(f*78), int(f*206), int(f*164)]
                transform = Compose([
                ...
                transformed_data_dict = transform(data_dict)
                return transformed_data_dict['image'], transformed_data_dict['label'], flag   
            elif self.monai_transform=="no_resize":
