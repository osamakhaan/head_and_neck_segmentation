...
from dataset_data_parallel import DatasetStg1
...
def get_arguments():
    ...
    parser.add_argument('--transform', type=str, default="crop_same_size", help="[rand_affine|rand_elastic|rand_spatial_crop|rand_zoom|crop_same_size]")
...
def main():
    ...
    model = monai.networks.nets.BasicUNet(dimensions=3, in_channels=1, out_channels=10)
    ...
    traindataloader = torch.utils.data.DataLoader(traindataset,num_workers=0,batch_size=4*torch.cuda.device_count(), shuffle=True)
    logging.info(f"Distributed batch size = {4*torch.cuda.device_count()}")
